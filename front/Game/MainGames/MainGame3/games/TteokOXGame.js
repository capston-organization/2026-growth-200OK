...
class TteokOXGame extends Phaser.Scene {
  ...
  create() {
    ...
    this.scoreText = this.add.text(width - 50, 30, "맞힌 문제: 0", {
      fontSize: "24px",
      fill: "#4A6B8B",
      fontFamily: "Arial",
      fontStyle: "bold",
    ...
    this.add.circle(hammerRabbitX, hammerRabbitY, 120, 0xffb6c1, 0.8).setDepth(0);
    ...
    this.hammerRabbit.setInteractive({ useHandCursor: true }).on("pointerdown", function (ptr) {
      ptr.event.stopPropagation();
      this.selectAnswerWithPointer(ptr);
    }, this).on("pointerover", function () {
      if (this.isQuestionActive && !this.showResult) {
        this.tweens.add({ targets: this.hammerRabbit, scaleX: 1.1, scaleY: 1.1, duration: 150, ease: "Back.easeOut" });
      }
    }, this).on("pointerout", function () {
      if (!this.showResult) {
        this.tweens.add({ targets: this.hammerRabbit, scaleX: 1, scaleY: 1, duration: 150, ease: "Back.easeIn" });
      }
    }, this);
    ...
    var xSize = 120;
    this.blueXLayer.lineBetween(waterRabbitX - xSize / 2, waterRabbitY - xSize / 2, waterRabbitX + xSize / 2, waterRabbitY + xSize / 2);
    this.blueXLayer.lineBetween(waterRabbitX + xSize / 2, waterRabbitY - xSize / 2, waterRabbitX - xSize / 2, waterRabbitY + xSize / 2);
    ...
    this.waterRabbit.setInteractive({ useHandCursor: true }).on("pointerdown", function (ptr) {
      ptr.event.stopPropagation();
      this.selectAnswerWithPointer(ptr);
    }, this).on("pointerover", function () {
      if (this.isQuestionActive && !this.showResult) {
        this.tweens.add({ targets: this.waterRabbit, scaleX: 1.1, scaleY: 1.1, duration: 150, ease: "Back.easeOut" });
      }
    }, this).on("pointerout", function () {
      if (!this.showResult) {
        this.tweens.add({ targets: this.waterRabbit, scaleX: 1, scaleY: 1, duration: 150, ease: "Back.easeIn" });
      }
    }, this);
  ...
  createSounds() {
    ...
    if (this.cache.audio.exists("three")) this.threeSound = this.sound.add("three");
    if (this.cache.audio.exists("four")) this.fourSound = this.sound.add("four");
  }
  ...
  showQuestion() {
    ...
    this.time.delayedCall(this.rhythmStartTime, this.startRhythmBeats, [], this);
    this.time.delayedCall(this.rhythmEndTime + 200, this.checkQuestionResult, [], this);
  }

  startCountBeats() {
    this.playCountSound(1);
    this.time.delayedCall(this.beatInterval, function () { this.playCountSound(2); }, [], this);
    this.time.delayedCall(this.beatInterval * 2, function () { this.playCountSound(3); }, [], this);
    this.time.delayedCall(this.beatInterval * 3, function () { this.playCountSound(4); }, [], this);
  }
  ...
  startRhythmBeats() {
    ...
    this.playHeySound();
    this.time.delayedCall(this.beatInterval, function () { this.playHeySound(); if (this.beatInputs[2]) this.beatInputs[2].time = this.time.now; }, [], this);
    this.time.delayedCall(this.beatInterval * 2, function () { this.playHeySound(); if (this.beatInputs[3]) this.beatInputs[3].time = this.time.now; }, [], this);
    this.time.delayedCall(this.beatInterval * 3, function () { this.playHeySound(); if (this.beatInputs[4]) this.beatInputs[4].time = this.time.now; }, [], this);
  }
  ...
  selectAnswer(answer) {
    ...
    if (timeSinceStart < this.rhythmStartTime || timeSinceStart > this.rhythmEndTime) return;
    ...
    if (!this.beatInputs[assignedBeat]) this.beatInputs[assignedBeat] = { time: this.time.now, answered: false, answer: null };
