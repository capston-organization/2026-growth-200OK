...
class ShortAnswerGame extends Phaser.Scene {
  ...
  create() {
    ...
    if (this.cache.audio.exists("donutBgMusic")) {
      ...
      var playBg = function () {
        ...
        if (this.sound.context.state === "suspended") {
          this.sound.context.resume().then(function () { this.backgroundMusic.play(); }.bind(this));
        } else {
    ...
    this.scoreText = this.add.text(width - 30, 30, "맞힌 문제: 0", {
      fontSize: "24px",
      fill: "#FF6B9D",
      fontFamily: "Arial",
      fontStyle: "bold",
    ...
    ).on("pointerdown", this.submitAnswer, this).on("pointerover", function () {
    ...
    this.add.text(width / 2, submitY, "제출", {
      fontSize: "28px",
      fill: "#FFFFFF",
      fontFamily: "Arial",
      fontStyle: "bold",
    ...
    var donutScale = 0.45;
    var donutGap = 130;
    this.add.image(-60, donutY, "donut").setOrigin(0.5, 1).setScale(donutScale).setDepth(0);
    this.add.image(-60, donutY - donutGap, "donut").setOrigin(0.5, 1).setScale(donutScale).setFlipX(true).setDepth(0);
    this.add.image(width + 60, donutY, "donut").setOrigin(0.5, 1).setScale(donutScale).setDepth(0);
    this.add.image(width + 60, donutY - donutGap, "donut").setOrigin(0.5, 1).setScale(donutScale).setFlipX(true).setDepth(0);
  ...
  submitAnswer() {
    ...
    if (isCorrect) {
      ...
      this.inputBackground.strokeRoundedRect(this.width / 2 - 250, this.inputY - 40, 500, 80, 15);
      for (var a = 0; a < this.aliens.length; a++) this.aliens[a].setTexture("alienHappy");
      this.tweens.add({ targets: [this.inputDisplayText], scaleX: 1.1, scaleY: 1.1, duration: 200, yoyo: true, ease: "Power2" });
    } else {
      ...
      this.inputBackground.strokeRoundedRect(this.width / 2 - 250, this.inputY - 40, 500, 80, 15);
      for (var b = 0; b < this.aliens.length; b++) this.aliens[b].setTexture("alienAngry");
    }
